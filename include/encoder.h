...
class Encoder {
public:
	static int decode(const char* buf, uint32_t len, void* out, TYPE type, uint32_t* strLen = NULL);
	static int encode(const void* value, char* out, uint32_t* strLen, TYPE type, uint32_t len = 0);
private:
